...
const WORKFLOW_META: Record<AuditWorkflow, { label: string; icon: typeof Stethoscope; badgeClass: string }> = {
  extraction: { label: "Extraction", icon: Stethoscope, badgeClass: "border-emerald-200 bg-emerald-50 text-emerald-700" },
  chart_note: { label: "Chart Note", icon: ClipboardList, badgeClass: "border-sky-200 bg-sky-50 text-sky-700" },
  chat: { label: "Chat", icon: MessageSquareText, badgeClass: "border-violet-200 bg-violet-50 text-violet-700" },
};

const STATUS_META: Record<AuditRunStatus, { label: string; badgeClass: string }> = {
  running: { label: "Running", badgeClass: "border-amber-200 bg-amber-50 text-amber-700" },
  completed: { label: "Completed", badgeClass: "border-emerald-200 bg-emerald-50 text-emerald-700" },
  failed: { label: "Failed", badgeClass: "border-rose-200 bg-rose-50 text-rose-700" },
};
...
const linkedRunEntries = (processedDocument?: ProcessedDocument | null) => {
  ...
  if (processedDocument?.auditRunId) {
    entries.push({ label: "Current extraction", runId: processedDocument.auditRunId });
  }
...
const SummaryStat = ({ label, value, tone = "default" }: { label: string; value: number; tone?: "default" | "success" | "warning" | "danger" }) => {
  const toneClass =
    tone === "success"
...
const AuditTrailSheet = ({ documentId, processedDocument, trigger }: AuditTrailSheetProps) => {
  ...
  useEffect(() => {
    ...
    fetchAuditRuns({ documentId, workflow: workflowFilter, status: statusFilter, limit: 150 })
      .then((payload) => {
        if (!cancelled) setRuns(payload);
      })
      .catch((error) => {
        if (!cancelled) {
          setRuns([]);
  ...
  const filteredRuns = useMemo(() => {
    ...
    return runs.filter((run) => {
      const haystack = [run.title, run.requestId, run.runId, run.error, run.actor, run.workflow].filter(Boolean).join(" ").toLowerCase();
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        {trigger ? trigger : (
          <Button variant="outline" size="sm" className="rounded-full border-slate-200 bg-white text-slate-700 hover:bg-slate-50" disabled={!documentId}>
      ...
      <SheetContent side="right" className="w-full overflow-hidden border-l border-slate-200 bg-[#f5f7fb] p-0 sm:max-w-[min(94vw,1160px)]">
        <div className="flex h-full min-h-0 flex-col">
          ...
          <div className="min-h-0 flex-1 bg-[#f5f7fb]">
            <ScrollArea className="h-full">
              <div className="p-4 md:p-5">
                {isLoadingRuns ? (
                  ...
                  <div className="mb-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">{runsError}</div>
                ) : null}

                {filteredRuns.length > 1 ? (
                  <div className="mb-4 rounded-2xl border border-slate-200 bg-white p-3 shadow-[0_8px_24px_rgba(15,23,42,0.04)]">
                    <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500">Run History</div>
                    <div className="flex flex-wrap gap-2">
                      {filteredRuns.map((run) => {
                        ...
                        return (
                          ...
                          >
                            <div className="max-w-[260px] truncate text-sm font-medium">{run.title || `${workflow.label} run`}</div>
                            <div className={cn("mt-1 text-xs", selected ? "text-slate-300" : "text-slate-500")}>{formatDateTime(run.startedAt)}</div>
                            <div className="mt-2 flex flex-wrap gap-2">
                              <Badge className={cn(selected ? "border-white/20 bg-white/10 text-white" : workflow.badgeClass)} variant="outline">
                                {workflow.label}
                              </Badge>
                              <Badge className={cn(selected ? "border-white/20 bg-white/10 text-white" : status.badgeClass)} variant="outline">
                                {status.label}
                              </Badge>
                            </div>
                          </button>
                              ...
                              <div className="flex items-start gap-3">
                                ...
                                <div className="min-w-0">
                                  <div className="flex flex-wrap items-center gap-2">
                                    {workflowMeta ? <Badge className="border-white/15 bg-emerald-400/10 text-emerald-200" variant="outline">{workflowMeta.label}</Badge> : null}
                                    {statusMeta ? <Badge className="border-white/15 bg-white/8 text-slate-100" variant="outline">{statusMeta.label}</Badge> : null}
                                  </div>
                    ...
                    {isLoadingDetail ? (
                      ...
                      <Tabs defaultValue="timeline" className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_8px_24px_rgba(15,23,42,0.04)]">
                        <TabsList className="grid h-auto grid-cols-3 rounded-xl bg-slate-100 p-1">
                          ...
                          <TabsTrigger value="failure" className="rounded-lg">Failure</TabsTrigger>
                        </TabsList>
                        ...
                        <TabsContent value="failure" className="mt-4">
                          {selectedRun.status === "failed" || selectedRun.error || failureEvents.length > 0 ? (
                            <div className="space-y-4">
                              ...
                              {failureEvents.length ? failureEvents.map((event) => (
                                <div key={event.id} className={cn("rounded-xl border p-4", getEventTone(event))}>
                                  <div className="flex items-start justify-between gap-3">
                                    ...
                                    <Badge
                                      variant="outline"
                                      className={cn(event.status === "error" ? "border-rose-300 bg-rose-100 text-rose-700" : "border-amber-300 bg-amber-100 text-amber-700")}
                                    >
