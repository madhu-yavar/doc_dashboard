...
class FunctionalStatusExtractorSkill {
  ...
  async execute(context) {
    ...
    if (!result.success) {
      return { success: false, step: "functional_status_extractor", error: result.error, usage: result.usage };
    }
    ...
    try {
      ...
      return { success: false, step: "functional_status_extractor", error: e.message, usage: result.usage };
    }
  }
}
