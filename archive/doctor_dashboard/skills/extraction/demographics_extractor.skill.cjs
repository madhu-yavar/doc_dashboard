...
class DemographicsExtractorSkill {
  ...
  async execute(context) {
    ...
    if (!result.success) {
      return { success: false, step: "demographics_extractor", error: result.error, usage: result.usage };
    }
    ...
    try {
      ...
      return { success: false, step: "demographics_extractor", error: e.message, usage: result.usage };
    }
  }
}
