...
class DocumentAnalyzerSkill {
  ...
  async execute(context) {
    ...
    if (!result.success) {
      return { success: false, step: "document_analyzer", error: result.error, usage: result.usage };
    }
    ...
    try {
      ...
      return { success: false, step: "document_analyzer", error: e.message, rawContent: result.content, usage: result.usage };
    }
  }
}
