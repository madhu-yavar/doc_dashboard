...
class ChartNoteAgent {
  constructor(config = {}) {
    ...
    this.config = {
      ...
      logSteps: true,
      parallelSectionConcurrency: this.parsePositiveInt(
        config.parallelSectionConcurrency || process.env.CHART_NOTE_PARALLEL_CONCURRENCY,
        4
      ),
      ...config
    };
  ...
  async execute(context, onProgress = null) {
    ...
    try {
      ...
      reasoningSteps.push(this.createReasoningStep(1, "clinical_analysis", analysisStep));
      ...
      reasoningSteps.push(this.createReasoningStep(2, "soap_structure", structureStep));
      ...
      const parallelStepDefs = [
        {
          order: 3,
          key: "allergies",
          progressStep: "allergies",
          label: "ALLERGIES section",
          run: () => this.generateAllergies(extractedData),
        },
        {
          order: 4,
          key: "subjective",
          progressStep: "subjective",
          label: "CHIEF COMPLAINT & HISTORY section",
          run: () => this.generateSubjective(extractedData, structureStep.subjective),
        },
        {
          order: 5,
          key: "comorbidities",
          progressStep: "comorbidities",
          label: "COMORBIDITIES section",
          run: () => this.generateComorbidities(extractedData),
        },
        {
          order: 6,
          key: "objective",
          progressStep: "objective",
          label: "PHYSICAL EXAMINATION section",
          run: () => this.generateObjective(extractedData, structureStep.objective),
        },
        {
          order: 7,
          key: "procedures",
          progressStep: "procedures",
          label: "PROCEDURES & INTERVENTIONS section",
          run: () => this.generateProcedures(extractedData),
        },
        {
          order: 9,
          key: "assessment",
          ...
          run: () => this.generateAssessment(extractedData, structureStep.assessment),
        },
        {
          order: 10,
          key: "pending",
          progressStep: "pending",
          label: "PENDING INVESTIGATIONS section",
          run: () => this.generatePendingInvestigations(extractedData),
        },
        {
          order: 11,
          key: "plan",
          progressStep: "plan",
          label: "PLAN section",
          run: () => this.generatePlan(extractedData, structureStep.plan),
        },
        {
          order: 12,
          key: "nursing",
          progressStep: "nursing",
          label: "NURSING CARE NEEDS section",
          run: () => this.generateNursingCare(extractedData),
        },
        {
          order: 13,
          key: "riskFlags",
          progressStep: "risk_flags",
          label: "RISK FLAGS section",
          run: () => this.generateRiskFlags(extractedData),
        },
      ];

      const parallelResults = await this.runWithConcurrency(
        parallelStepDefs,
        this.config.parallelSectionConcurrency,
        async (stepDef) => {
          console.log(`📝 Step ${stepDef.order}: Generating ${stepDef.label}...`);
          const result = await stepDef.run();
          if (onProgress) onProgress({ step: stepDef.progressStep, status: "complete" });
          return {
            ...stepDef,
            result,
          };
      ...
      const sectionResults = Object.fromEntries(parallelResults.map((entry) => [entry.key, entry.result]));
      parallelResults
        .sort((left, right) => left.order - right.order)
        .forEach((entry) => {
          reasoningSteps.push(this.createReasoningStep(entry.order, entry.key, entry.result));
      ...
      const hospitalCourseStep = await this.generateHospitalCourse(
        extractedData,
        sectionResults.subjective?.content
      );
      reasoningSteps.push(this.createReasoningStep(8, "hospital_course", hospitalCourseStep));
      ...
      const reviewStep = await this.reviewAndRefine({
        allergies: sectionResults.allergies?.content,
        subjective: sectionResults.subjective?.content,
        comorbidities: sectionResults.comorbidities?.content,
        objective: sectionResults.objective?.content,
        procedures: sectionResults.procedures?.content,
        hospitalCourse: hospitalCourseStep.content,
        assessment: sectionResults.assessment?.content,
        pending: sectionResults.pending?.content,
        plan: sectionResults.plan?.content,
        nursing: sectionResults.nursing?.content,
        riskFlags: sectionResults.riskFlags?.content,
        validationSummary
      });
      reasoningSteps.push(this.createReasoningStep(14, "review", reviewStep));
      totalTokens += reviewStep.usage?.totalTokens || 0;

      if (onProgress) onProgress({ step: "review", status: "complete" });

      // Compile final chart note
      const orderedReasoningSteps = [...reasoningSteps].sort((left, right) => (left.order || 0) - (right.order || 0));
      const finalChartNote = this.compileChartNote({
        allergies: reviewStep.refined?.allergies || sectionResults.allergies?.content,
        subjective: reviewStep.refined?.subjective || sectionResults.subjective?.content,
        comorbidities: reviewStep.refined?.comorbidities || sectionResults.comorbidities?.content,
        objective: reviewStep.refined?.objective || sectionResults.objective?.content,
        procedures: reviewStep.refined?.procedures || sectionResults.procedures?.content,
        hospitalCourse: reviewStep.refined?.hospitalCourse || hospitalCourseStep.content,
        assessment: reviewStep.refined?.assessment || sectionResults.assessment?.content,
        pending: reviewStep.refined?.pending || sectionResults.pending?.content,
        plan: reviewStep.refined?.plan || sectionResults.plan?.content,
        nursing: reviewStep.refined?.nursing || sectionResults.nursing?.content,
        riskFlags: reviewStep.refined?.riskFlags || sectionResults.riskFlags?.content,
        extractedData,
        validationSummary
      });
      ...
      return {
        ...
        data: {
          ...
          reasoning_steps: orderedReasoningSteps,
          metadata: {
            generated_at: new Date().toISOString(),
            total_tokens: totalTokens,
            generation_time_ms: elapsed,
            agent_type: "react",
            steps_completed: orderedReasoningSteps.length
          }
        },
        usage: { totalTokens }
      };
  ...
  createReasoningStep(order, step, result = {}) {
    return {
      order,
      step,
      ...result,
