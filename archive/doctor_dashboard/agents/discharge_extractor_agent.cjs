...
class DischargeExtractorAgent {
      ...
      if (onProgress) {
        onProgress({
          ...
          data: {
            ...
            latency: normalizedResult.usage?.latencyMs || normalizedResult.usage?.latency || 0,
            queueWaitMs: normalizedResult.usage?.queueWaitMs || 0,
  ...
  serializeStepSummary(step) {
    return {
      ...
      queueWaitMs: step.usage?.queueWaitMs || 0,
  ...
  assembleFinalResult(steps, pdfName) {
    ...
    steps.forEach(step => {
      if (step.success && step.data) {
        ...
        if (step.step === "pending_items_extractor") {
