  ...
  async execute({ document, documentId, message, sectionContext, chatId, geminiApiKey = "" }) {
    ...
    if (classification.needsExternal) {
      this.pushTrace(
        ...
        "external_search",
        "External Search",
        useGeminiWebSearch ? "ok" : "warning",
        useGeminiWebSearch
          ? "Primary external medical search uses Gemini grounded web search. Alternate fallback search/model is available if Gemini is unavailable."
          : "Gemini grounded web search is unavailable for this turn. Falling back to approved external sources with alternate model synthesis.",
        {
          provider: useGeminiWebSearch ? "gemini_web" : "gemma_external_fallback",
        }
      );
    } else {
    ...
    if ((classification.intent === "medication_comparison" || classification.intent === "medication_substitution") && !externalEvidence.length && comparisonResolution?.answer) {
      ...
      answerPayload = (
        await this.answerAgent.execute({
          ...
          externalComposer: classification.needsExternal
            ? useGeminiWebSearch
              ? "gemini_web"
              : "gemma_external_fallback"
            : "gemma",
          geminiApiKey: transientGeminiApiKey || this.defaultGeminiApiKey,
        })
      ).data;
      traceProvider = answerPayload.llm_provider || "gemma";
      if (answerPayload.fallback_meta?.alternate_provider) {
        this.pushTrace(
          trace,
          "fallback",
          "Fallback Plan B",
          "warning",
          `Primary external model was unavailable. Returned the answer via ${answerPayload.fallback_meta.alternate_provider}.`,
          {
            reason: answerPayload.fallback_meta.reason,
            alternate_provider: answerPayload.fallback_meta.alternate_provider,
          }
        );
      }
    }
