...
class AnswerComposerAgent {
  constructor(config = {}) {
    ...
    this.externalKnowledgeAgent = new ExternalKnowledgeAgent(config);
  }

  isGeminiCapacityError(error = "") {
    return /\b503\b|high demand|temporar(?:y|ily)|unavailable|status\"\s*:\s*\"UNAVAILABLE/i.test(String(error || ""));
  }

  prependNotice(notice, answer = "") {
  ...
  async buildGeminiGroundedFallback({
    message,
    classification,
    internalEvidence = [],
    externalEvidence = [],
    ...
    const fallbackGeminiResult = await this.geminiClient.executeGroundedSearch(message, {
      apiKey: externalMeta?.geminiApiKey || "",
      systemInstruction: this.promptBuilder.buildGeminiExternal({
        message,
        classification,
        chatHistory,
      }).systemInstruction,
      temperature: 0.1,
      maxTokens: 500,
      model: this.geminiClient.fallbackModel,
    });

    const primaryServiceNotice = this.isGeminiCapacityError(geminiError)
      ? "Primary Gemini grounded search is temporarily unavailable due to capacity."
      : "Primary Gemini grounded search is temporarily unavailable right now.";

    if (fallbackGeminiResult.success && fallbackGeminiResult.content.trim()) {
      return {
        answer: this.prependNotice(
          `${primaryServiceNotice} I switched to the alternate Gemini model ${fallbackGeminiResult.model} for this answer.`,
          this.compactSentences(fallbackGeminiResult.content, 2, 360)
        ),
        citations: this.citationAssembler.assemble(fallbackGeminiResult.citations || [], { max: 4 }),
        source_class: "external",
        llm_provider: "gemini_web_fallback",
        confidence_override: 82,
        confidence_label_override: "high",
        refused_override: false,
        fallback_meta: {
          reason: "gemini_grounded_unavailable",
          alternate_provider: fallbackGeminiResult.model,
          gemini_error: geminiError || undefined,
        },
      };
    }

    let fallbackEvidence = Array.isArray(externalEvidence) ? externalEvidence : [];
    let fallbackMeta = externalMeta || {};

    if (!fallbackEvidence.length) {
      const fallbackResult = await this.externalKnowledgeAgent.execute({
        query: message,
        classification,
        internalEvidence,
      });
      ...
      fallbackEvidence = Array.isArray(fallbackData.evidence) ? fallbackData.evidence : [];
      fallbackMeta = {
        ...fallbackMeta,
        resolution: fallbackData.resolution || fallbackMeta.resolution || null,
        plan: fallbackData.plan || fallbackMeta.plan || null,
        error: fallbackData.error || fallbackMeta.error || null,
        error_type: fallbackData.error_type || fallbackMeta.error_type || null,
      };
    }

    const serviceNotice = this.isGeminiCapacityError(geminiError)
      ? `Primary Gemini grounded search is temporarily unavailable due to capacity, and alternate model ${this.geminiClient.fallbackModel} also did not return a usable grounded answer.`
      : `Primary Gemini grounded search is temporarily unavailable, and alternate model ${this.geminiClient.fallbackModel} also did not return a usable grounded answer.`;

    if (fallbackEvidence.length) {
      const sourceClass =
        internalEvidence.length && fallbackEvidence.length
          ? "mixed"
      ...
      const citationItems =
        sourceClass === "mixed"
          ? [internalEvidence[0]].filter(Boolean).concat(fallbackEvidence, internalEvidence.slice(1))
          : [...internalEvidence, ...fallbackEvidence];

      let synthesizedAnswer = "";

      if (classification?.intent === "diagnosis_code") {
        synthesizedAnswer = this.buildCodingAnswer(message, fallbackEvidence)?.answer || "";
      } else if (classification?.intent === "clinical_explanation") {
        synthesizedAnswer = this.buildClinicalExplanationAnswer(message, internalEvidence, fallbackEvidence)?.answer || "";
      } else if (
        classification?.intent === "drug_safety" ||
        fallbackMeta?.resolution?.generic_name ||
        fallbackMeta?.resolution?.normalized_display
      ) {
        synthesizedAnswer =
          this.buildDrugKnowledgeAnswer(message, internalEvidence, fallbackEvidence, fallbackMeta?.resolution || null)?.answer || "";
      }
      ...
      if (!synthesizedAnswer) {
        const prompt = this.promptBuilder.build({
          ...
          classification,
          internalEvidence,
          externalEvidence: fallbackEvidence,
          chatHistory,
        });

        const gemmaResult = await this.gemmaClient.execute(prompt, { temperature: 0.1, maxTokens: 600 });
        if (gemmaResult.success && gemmaResult.content.trim()) {
          synthesizedAnswer = gemmaResult.content.trim();
        }
      }

      if (!synthesizedAnswer) {
        synthesizedAnswer =
          this.buildExternalOnlyFallback(message, fallbackEvidence, fallbackMeta?.resolution || null)?.answer || "";
      }

      if (synthesizedAnswer) {
        return {
          answer: this.prependNotice(
            `${serviceNotice} I used approved external references with Gemma as a fallback synthesizer for this answer.`,
            synthesizedAnswer
          ),
          ...
          llm_provider: "gemma_external_fallback",
          confidence_override: 72,
          confidence_label_override: "medium",
          refused_override: false,
          fallback_meta: {
            reason: "gemini_grounded_unavailable",
            alternate_provider: "gemma",
            gemini_error: geminiError || undefined,
            fallback_gemini_error: fallbackGeminiResult.error || undefined,
    ...
    if (internalEvidence.length) {
      return {
        answer: this.prependNotice(
          `${serviceNotice} I could not complete grounded external search, so this answer is limited to the uploaded record.`,
          `Based on the available patient record, the most relevant findings are: ${internalEvidence
            .slice(0, 3)
            .map((item) => item.value)
            .join("; ")}.`
        ),
        citations: this.citationAssembler.assemble(internalEvidence, { max: 3 }),
        source_class: "internal",
        llm_provider: "internal_fallback",
        confidence_override: 65,
        confidence_label_override: "low",
        refused_override: false,
        fallback_meta: {
          reason: "gemini_grounded_unavailable",
          alternate_provider: "internal_record_only",
          gemini_error: geminiError || undefined,
          fallback_gemini_error: fallbackGeminiResult.error || undefined,
          evidence_count: internalEvidence.length,
        },
      };
    }
    ...
    return {
      answer: this.prependNotice(
        serviceNotice,
        "I could not produce a reliable fallback answer from approved sources right now. Please try again later."
      ),
      citations: [],
      source_class: "external",
  ...
  async execute({
    ...
    if (externalComposer === "gemini_web") {
      ...
      const fallback = await this.buildGeminiGroundedFallback({
        message,
        classification,
        internalEvidence,
        externalEvidence,
    ...
    if (externalComposer === "gemma_external_fallback") {
      return {
        success: true,
        step: "answer_composer",
        data: await this.buildGeminiGroundedFallback({
          message,
          classification,
          internalEvidence,
          externalEvidence,
          ...
          chatHistory,
          geminiError: "Gemini grounded web search is unavailable for this turn.",
        }),
      };
    }
