...
const Index = () => {
  ...
  const extractionEscalation = processedDocument?.result?.meta?.extraction_escalation;
  ...
  const escalationBanner = extractionEscalation?.required ? (
    <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
        <span className="font-semibold">Review required</span>
        <span>
          Deviation {extractionEscalation.deviation_pct || 0}% ({extractionEscalation.reliable_items || 0}/{extractionEscalation.total_items || 0} core sections reliable)
        </span>
        <span>Threshold {extractionEscalation.threshold_pct || 50}%</span>
      </div>
      <p className="mt-1 text-amber-800">
        {extractionEscalation.plan_b?.summary || "More than half of the core dashboard sections were weak or missing."}
      </p>
      {Array.isArray(extractionEscalation.weak_items) && extractionEscalation.weak_items.length > 0 ? (
        <p className="mt-1 text-amber-800">
          Weak sections: {extractionEscalation.weak_items.join(", ")}
        </p>
      ) : null}
    </div>
  ) : null;
  ...
  if (activeSection === "vitals") return <PageWrapper>{pageHeader}<VitalsDetail onBack={handleBack} data={d} />{assistantPanel}</PageWrapper>;
  if (activeSection === "diagnosis") return <PageWrapper>{pageHeader}<DiagnosisDetail onBack={handleBack} data={d} />{assistantPanel}</PageWrapper>;
  if (activeSection === "medications") return <PageWrapper>{pageHeader}<MedicationsDetail onBack={handleBack} data={d} />{assistantPanel}</PageWrapper>;
  if (activeSection === "labs") return <PageWrapper>{pageHeader}<LabsDetail onBack={handleBack} data={d} />{assistantPanel}</PageWrapper>;
  if (activeSection === "radiology") return <PageWrapper>{pageHeader}<RadiologyDetail onBack={handleBack} data={d} />{assistantPanel}</PageWrapper>;
  if (activeSection === "treatment") return <PageWrapper>{pageHeader}<TreatmentDetail onBack={handleBack} data={d} />{assistantPanel}</PageWrapper>;
  if (activeSection === "notes") return <PageWrapper>{pageHeader}<ClinicalNotesDetail onBack={handleBack} data={d} />{assistantPanel}</PageWrapper>;
  if (activeSection === "discharge") return <PageWrapper>{pageHeader}<DischargeDetail onBack={handleBack} data={d} />{assistantPanel}</PageWrapper>;
  if (activeSection === "followup") return <PageWrapper>{pageHeader}<FollowUpDetail onBack={handleBack} data={d} />{assistantPanel}</PageWrapper>;
  if (activeSection === "pending") return <PageWrapper>{pageHeader}<PendingItemsDetail onBack={handleBack} data={d} />{assistantPanel}</PageWrapper>;
  if (activeSection === "riskwatch") return <PageWrapper>{pageHeader}<RiskWatchDetail onBack={handleBack} data={d} />{assistantPanel}</PageWrapper>;
