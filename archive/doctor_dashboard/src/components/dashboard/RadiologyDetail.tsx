...
const RadiologyDetail = ({ onBack, data }: RadiologyDetailProps) => {
  const { radiology } = data;
  const radiologyProvenance = data.provenance.sections.radiology;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <SectionProvenanceBadge status={radiologyProvenance.status} />
        {radiology.criticalFindings > 0 ? (
          <StatusBadge status="critical" label={`${radiology.criticalFindings} Critical Finding`} />
        ) : null}
      </div>

      <ProvenancePanel status={radiologyProvenance.status} items={radiologyProvenance.items} />
      ...
      {radiology.studies.length > 0 ? (
        radiology.studies.map((study, i) => (
          <div key={i} className={`bg-card rounded-xl border p-5 ${study.critical ? "border-status-critical/30" : ""}`}>
            <div className="flex items-center justify-between mb-3">
              ...
              {study.critical && <StatusBadge status="critical" label="Critical" />}
            </div>
            <p className="text-xs text-muted-foreground mb-4">
              {study.date ? `Date: ${study.date}` : "Date not documented"}
              {study.performedBy ? ` · ${study.performedBy}` : ""}
            </p>
        ...
        <div className="bg-muted/30 rounded-lg p-8 text-center text-sm text-muted-foreground">
          No source-backed radiology findings were documented in this record.
        </div>
      )}

      {/* Pending */}
      {radiology.pending.length > 0 ? (
        <div className="bg-card rounded-xl border p-5">
          <h3 className="font-semibold text-sm mb-3 text-foreground">⏳ Imaging Orders / Pending Studies</h3>
