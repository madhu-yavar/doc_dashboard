#!/bin/bash
# Production Prescription Fix - Deploy via SSH
# This script will SSH into the production server and apply the fix
...
echo "🚀 Deploying Prescription Fix to Production"
echo "=========================================="
echo "VM: $VM_USER@$VM_IP"
echo ""

# Create SSH command with timeout
SSH_CMD="ssh -i $SSH_KEY -o StrictHostKeyChecking=no -o ConnectTimeout=30 $VM_USER@$VM_IP"
...
if $SSH_CMD "echo 'SSH connection successful'"; then
    ...
    echo "1. SSH key is in the correct location"
    echo "2. Key has proper permissions (chmod 600)"
    echo "3. Server is accessible"
    exit 1
fi

echo ""
echo "📋 Deploying prescription templates..."
echo ""

# Execute the fix on the remote server
$SSH_CMD bash << 'ENDSSH'
...
echo ""
echo "🎉 Prescription fix deployed!"
echo ""
